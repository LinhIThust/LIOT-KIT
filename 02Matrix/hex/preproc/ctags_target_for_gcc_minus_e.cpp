# 1 "c:\\Users\\Dong Quang Linh\\OneDrive\\Example_LIOT_KIT\\02Matrix\\Matrix.ino"
// void setup()
// {
// 	Serial.begin(115200);
...
# 74 "c:\\Users\\Dong Quang Linh\\OneDrive\\Example_LIOT_KIT\\02Matrix\\Matrix.ino"
...
# 86 "c:\\Users\\Dong Quang Linh\\OneDrive\\Example_LIOT_KIT\\02Matrix\\Matrix.ino"
void displayImage(unsigned int image[],int duration)
{
  for(int hold=0;hold<duration;hold++)
  {
    for(int a=0;a<8;a++)
    {
      digitalWrite(LATCH, 0x0);
      shiftOut(DATA, CLOCK, 1,~image[a]);//column
      shiftOut(DATA, CLOCK, 1,row[a]);//row
      digitalWrite(LATCH, 0x1);
      delay(1);
    }
  }
}
...
void clearImage(int duration)
{
  for(int hold=0;hold<duration;hold++)
  {
    ...
    {
      digitalWrite(LATCH, 0x0);
      shiftOut(DATA, CLOCK, 1,255);//column
      shiftOut(DATA, CLOCK, 1,row[a]);//row
      digitalWrite(LATCH, 0x1);
      delay(1);
    }
  ...
  for(shift = 0; shift < 9; shift++)
  {
    for(hold = 0; hold < 30; hold++)
    {
      for(a = 0; a < 8; a++)
      {
        ...
        shiftOut(DATA,CLOCK,1,~(image[a]<<shift));//column
        shiftOut(DATA,CLOCK,1,row[a]);//row
...
# 159 "c:\\Users\\Dong Quang Linh\\OneDrive\\Example_LIOT_KIT\\02Matrix\\Matrix.ino"
